...
%PTE = (H  => [1 ,  1.007825], He => [2 ,  4.002603], Li => [3 ,   7.016004], Be => [4 ,  9.012182], B  => [5 , 11.009305], C  => [6 , 12.000000], N  => [7 , 14.003074], O  => [8 , 15.994914], F  => [9 , 18.998403], Ne => [10, 19.992440], Na => [11, 22.989769], Mg => [12, 23.985041], Al => [13, 26.981538], Si => [14, 27.976926], P  => [15, 30.973761], S  => [16, 31.972071], Cl => [17, 34.968852], Ar => [18, 39.962383], K => [19, 38.963706], Ca => [20,  39.962590]);
...
for my $i (0..(scalar(@hmlinks)+scalar(@hllinks)-1)){
	printf XYZ "%s    %12.6f  %12.6f  %12.6f\n", 'H', $H_pos[3*$i], $H_pos[3*$i+1], $H_pos[3*$i+2];
        printf PDB1 "ATOM  %5d %3s  LIG     1    %8.3f%8.3f%8.3f\n", $high_count, 'H', $H_pos[3*$i], $H_pos[3*$i+1], $H_pos[3*$i+2];
}
close(XYZ);
close(XYZ1);
...
if ($link_atoms eq '0'){
	...
	print OUT "BOND\n";
	while (<IN>){
		if (/^\s*Could not find bond parameter for atom types:\s+(\w.*)/){
			my @tmp = split(/\s+/,$1);
			if (grep {$_ eq 'AL'} @tmp){
			} else {
	...
	print OUT "\nANGLE\n";
	while (<IN>){
	        if (/^\s*Could not find angle parameter for atom types:\s+(\w.*)/){
			my @tmp = split(/\s+/,$1);
                        if (grep {$_ eq 'AL'} @tmp){
                        } else {
	...
	print OUT "\nDIHE\n\n";
	while (<IN>){
                if (/^\s*Could not find dihedral parameter for atom types:\s+(\w.*)/){
                        my @tmp = split(/\s+/,$1);
                        if (grep {$_ eq 'AL'} @tmp){
                        } else {
